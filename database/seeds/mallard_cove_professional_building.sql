...
-- Run database/schema.sql first. This seed is idempotent for the property,
-- brokers, tenants, summaries, analysis, diligence items, seller weaknesses,
-- valuation scenarios, and broker questions.
...
INSERT INTO properties (
    ...
    price_per_sf,
    annual_base_rent,
    annual_opex_recovery,
    implied_total_expenses,
    implied_expenses_psf,
    implied_expense_ratio_pct,
    traffic_count_vpd,
    demographics,
    property_highlights,
    market_summary,
...
SELECT id, '9d5debbf-b7dd-4943-9e8d-e66df2b91f9b.pdf', 'offering_memorandum', 'application/pdf', 'processed', now(), '{"pages":11,"contains_rent_roll":true,"contains_tenant_profiles":true,"contains_site_plan":true,"contains_property_survey":true,"contains_market_overview":true}'::jsonb
FROM properties WHERE address = '2643-2751 S Loop 336 W, Conroe, TX 77304';

INSERT INTO brokers (property_id, broker_name, title, company, phone, phone_ext, email, office_address, website, source_page)
SELECT p.id, b.broker_name, b.title, b.company, b.phone, b.phone_ext, b.email, b.office_address, b.website, b.source_page
FROM properties p
CROSS JOIN (VALUES
    ('Linda Crumley', 'Advisor', 'SVN | J. Beard Real Estate', '281-367-2220', '119', 'linda.crumley@svn.com', '9320 Lakeside Blvd, Ste 250, The Woodlands, TX 77381', 'jbeardcompany.com', 11),
    ('Brigham Hedges', 'Associate Advisor', 'SVN | J. Beard Real Estate', '281-367-2220', '143', 'brigham.hedges@svn.com', '9320 Lakeside Blvd, Ste 250, The Woodlands, TX 77381', 'jbeardcompany.com', 11)
) AS b(broker_name, title, company, phone, phone_ext, email, office_address, website, source_page)
WHERE p.address = '2643-2751 S Loop 336 W, Conroe, TX 77304'
...
SELECT p.id, t.tenant_name, t.tenant_name, t.suite, t.building_address, t.unit_size_sf, t.unit_size_sf,
       ...
       t.tenant_quality, t.risk_notes, t.source_page
FROM properties p
CROSS JOIN (VALUES
    ...
    ('PowerPath International', 'Suite B', '2685 S Loop 336 W', 2500, 26.50, 'NNN', 5521, 66250, 13750, '2026-07-01'::date, '2029-06-30'::date, '3% Annual', 'None', 'Global supplier of power generation equipment and energy solutions.', 'powerpathie.com', 'local/specialty', 'Lease begins July 2026; verify commencement, guarantee, and payment history.', 4),
    ('Experience Network, LLC', 'Suite A', '2671 S Loop 336 W', 2500, 25.50, 'NNN', 5313, 63750, 13125, '2024-12-01'::date, '2029-11-30'::date, '$0.50 Annual', 'One 5-year option at market rate', 'Also known as Grand Terra Realty, an independent brokerage with 75 agents serving Montgomery and surrounding counties.', 'grandterrarealty.com', 'local', 'Local tenant profile; verify tenant financials and renewal probability.', 4),
    ('Quatro Tax, LLC', 'Suite B', '2671 S Loop 336 W', 2500, 25.50, 'NNN', 5313, 63750, 13125, '2025-10-01'::date, '2028-09-30'::date, '$0.50 Annual', 'No Option', 'Property tax consulting firm with offices in Conroe, Houston, and Dallas Fort Worth. OM says it manages ad valorem taxes for over $12.4B in real estate and business personal property.', 'quatrotax.com', 'stronger', 'No renewal option; OM text has typo in September expiration.', 4),
    ('Ryan Nelson Chiropractor', 'Suite A', '2657 S Loop 336 W', 2500, 25.20, 'Gross', 5250, 63000, NULL::numeric, '2023-02-01'::date, '2028-02-29'::date, '5% Annual', 'None', 'Chiropractic and rehabilitation practice focused on physiotherapy, stretching, strengthening exercises, spinal manipulation, exercise, and nutrition.', 'conroespine.com', 'local', 'Gross lease expense exposure; verify reimbursement obligations and tenant credit.', 4),
